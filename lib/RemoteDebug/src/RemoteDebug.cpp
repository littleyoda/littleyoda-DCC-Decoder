...
void RemoteDebug::setLogLevel(uint8_t loglevel) {
	Serial.println(String(loglevel) + " " + String(VERBOSE) + " " + String(ERROR));
	if (loglevel >= VERBOSE && loglevel <= ERROR) {
		_clientDebugLevel = loglevel;
		Serial.println("Setting to " + String(_clientDebugLevel));
	}
}

uint8_t RemoteDebug::getLogLevel() {
	return _clientDebugLevel;
}
